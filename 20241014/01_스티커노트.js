...
class StickerNote{
    ...
    toggle_note_list(){
        ...
        listButton.onclick = () => {
            ...
            if(!sideBar.hasAttribute('active')){
                // 사이브 바에 있는 모든 li들을 재구성하기 위해 전부 제거한다
                ...
                for(let i = 0; i < localStorage.length; i++){
                    ...
                    li.onclick = () => {
                        if (confirm('노트 내용을 변경하시겠습니까?')){
                            const key = li.textContent; // 노트제목을 가져온다 (key 값)
    ...
    move_note(){
        ...
        topBar.onmousedown = event => {
            // 내가 클릭한 위치 - 노트의 왼쪽 x축 위치 == 내가 노트로부터 얼만큼 띄워진 곳을 클릭했는가
            const shiftX = event.clientX - this.note.getBoundingClientRect().left;
            const shiftY = event.clientY - this.note.getBoundingClientRect().top;
